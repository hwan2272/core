...
gb_ExternalProject_libgpg-error_target := $(if $(filter INTEL,$(CPUNAME)),pe-i386,$(if $(filter ARM64,$(CPUNAME)),pe-arm64,pe-x86-64))
	...
	$(call gb_ExternalProject_run,build,\
		MAKE=$(MAKE) ./configure \
			...
			$(if $(CROSS_COMPILING), \
				...
				CC_FOR_BUILD="$(call gb_Executable_get_target_for_build,gcc-wrapper) --wrapper-env-prefix=REAL_BUILD_ $(SOLARINC)" \
			, \
	            --host=$(gb_ExternalProject_libgpg-error_host)) \
			RC='windres -O COFF --target=$(gb_ExternalProject_libgpg-error_target) --preprocessor='\''$(call gb_Executable_get_target_for_build,cpp) -+ -DRC_INVOKED -DWINAPI_FAMILY=0 $(SOLARINC)'\' \
	    && $(MAKE) \
	)
